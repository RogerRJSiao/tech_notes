...
### 1.1 測試模型基準
1. 知識問答、繁中理解
  - 資料集：TMMU+ (特色：台灣用語、選擇題)
  - 評估：準確率、速度
  - 應用：中文客服機器人，可用 deepseek-rl:14b、qwen3:14b
2. 多語系翻譯
  - 資料集：FLORES-200 (特色：有標準譯文)
  - 評估：chrF2 判斷字元重疊度、BERTScore 判斷詞向量接近度
  - 應用：長文本中文翻譯，可用 gemma3:12b-it-qat (128k 上下文)
3. 算術計算
  - 資料集：GSM8K (特色：小學程度數學題目)
  - 評估：準確率、速度
  - 應用：qwen3:14b、phi4:14b
4. 邏輯推論
  - 資料集：LogiQA (全英文，題目類型：條件推論、因果關係、集合歸納、矛盾檢測等)
  - 評估：準確率
  - 應用：可用 deepseek-rl:14b (用於小文本)、qwen3:14b (用於大文本)
5. 程式碼生成
  - 資料集：HumanEval (特色：python)
  - 評估：pass@1
  - 應用：本地端生成程式碼，可用 phi4:14b (用於小文本)、gemma3:12b-it-qat (用於大文本)
...
- **模型壓縮**
  - 量化(quantization)
    - 量化感知技術(quantization aware training)：在訓練階段帶入低位元限制，壓縮模型容量同時保留精度，比擬 BF16 精度，模型命名後墜 `-it-qat`。
