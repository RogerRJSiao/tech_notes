...
- [例行] 生成樹協定(Spanning Tree Protocol, STP)：實現網路備援(Network Redundancy)，提高網路可靠性。
...
- 資安裝置
    - 防火牆
        ...
        - 分區概念
            - 隔離區/半信任區(demilitarized zone, DMZ)：通常是web server和mail server。採用三介面防火牆(three-interface firewall)：
                - DMZ → Internal：全數阻擋。
                - DMZ → External：只允許必須的服務。
                - Internal → DMZ：檢查後允許回傳。採取 stateful inspection。
                - External → DMZ：只允許必須的服務。
        ...
        | 共通 | 感測器、使用特徵辨識惡意軟體 | 感測器、使用特徵辨識惡意軟體 |
...
- 資安服務
    - 網路存取控制清單(ACL)控制流量
        - 功能：設定轉送及丟棄封包規則，限制路由更新的傳遞，範圍存取可以被限制在有授權的用戶，根據流量型態過濾封包，根據服務種類分辨可否存取，區分流量給予部分優先通行權力，記錄符合規則的存取要求。
        - 設定：可用編號、名稱建立
            - 標準存取控制列表(standard ACLs)：只比對來源IPv4位址。
            - 擴展存取控制列表(extended ACLs)：通訊協定、來源IPv4及port、目的IPv4及port、更精細設定。
            - Cisco ACL可設定只允許已建立ACK或RST bitset的TCP流量可通行，防止外網想要建立新TCP連線。

    - 簡單網路管理協定(SNMP, Simple Network Management Protocol)-NMS
        - 設定：IP位址, port, read community, write community
        - 功能：管理者瀏覽MIB Tree，以遠端方式設定及管理裝置，如路由器、交換器、資安設備、工作站。監控並管理網路性能。
        - 組成：SNMP管理者(SNMP manager)以SNMP軟體管理，SNMP代理者(SNMP agent)是被監控和管理的節點。後者包括管理資料庫(Management Information Base, MIB)，使用樹狀結構，定義唯一的物件識別碼(OID)。
            - Get：管理者向代理者收集資訊。
            - Set：管理者向代理者設定資訊。
            - Trap：代理者向管理者發送資訊。

    - NetFlow Collecter
        - 匯入：來源IP和port、目的IP和port、L3通訊協定種類、ToS marking、輸入邏輯介面。封包數、遮罩。
        - 設定：管理者安裝NetFlow Collecter軟體，取得啟用NetFlow的路由器的出入流量統計資訊。
        - 功能：監控網路安全、流量分析，辨別網路瓶頸，為帳單計算的IP計量。

    - Port Mirroring
        - 功能：屬於封包分析(packet analyzer, packet sniffer, traffic sniffer)，抓取進出網卡的封包。允許交換器複製封包，提供IDS等其他服務的匯入來源，完成Traffic Sniffing。

    - Syslog Servers
        - 匯入：System Message包括時戳、主機位址、訊息，來源是支援Syslog的路由器、交換器、服務主機、防火牆等。
        - 功能：收集日誌可監測及排錯，選取日誌資訊種類，指定找出目標IP訊息。

    - 網路時間協定(NTP, Network Time Protocol)
        - 功能：NTP伺服器是透過階層式的stratum level定義，stratum 0是權威外部時間來源(authoritative time source)，每一個hop就加1，stratum 0與stratum 1 (primary network time standard)直接相連，Stratum 2 (lower strata)再與stratum 1相連，直到最大值15跳的Stratum 16 (unsynchronized)。

    - AAA伺服器 (Triple A, autentication, authorization, accounting)
        - 設定：用戶名、用戶IP、伺服器種類、金鑰，使用者帳號、密碼。
        - 實例 [常考]
            | 特性 | **TACACS+** | **RADIUS** |
            | ------- | ----- | ----- |
            | 協定類型 | TCP（port 49）| UDP（port 1812/1813 or 1645/1646） |
            | 安全性 | 將整個封包(包括認證、授權)加密 | 僅加密密碼欄位，其他資訊為明文   |
            | 支援   | Cisco為主        | 開源，RFC 2865標準   |
            | AAA分離 | 分離認證、授權與記錄 | 結合認證+授權，分離記錄  |
            | 客製化 | 根據使用者、群組提供路由器cmd授權 | 無法 |
            | 使用場景  | limited，網路設備管理員登入(管理存取控制)  | extensive，使用者遠端存取網路(如VPN、Wi-Fi、撥號) |
            | CHAP協定 | 雙向 | 單向(RADIUS資安主機->用戶) |

    - VPN (Virtual Private Network) = VPN with encryption
        - 設定：邏輯連線是以L2、L3 (如GRE、MPLS、IPsec)。
        - 功能：跨越公有網域建立邏輯連線，建立點對點連線(GRE、 IPsec)、任一連任一(MPLS)。IPsec服務可提供remote-access、site-to-site服務，達成認證、資訊完整、存取控制、機密。
    
    - IDS (intrusion detection systems)

    - SPAN (switch port analyzer)
        - Cisco獨有的，在交換器上將某個埠的流量複製到監控埠。
        - 與Network Tap同為把送量到監控設備。
...
| 華語 | ENG |
| --- | --- |
| 根據使用者、群組提供授權 | provide authorization on per-user or per-group basis |
| 裝置重啟 | a device reboots |
| 透過NTP伺服器同步時間設定 | synchronize their time settings with an NTP server |
| 高精確度計時裝置 | high-precision timekeeping devices |
| 權威外部時間來源 | authoritative time sources |
| 取得正確日期時戳 | get an accurate date and timestamp |
| 較好的解決方案是... | A better solution is to configure... |
| 複製流量 | make duplicate copies of traffic |
| NetFlow啟用路由器 | NetFlow Enabled Router |
| 首要必須的是定義管理者和代理者的關係 | it is first necessary to define the relationship between the manager and the agent. |
| 網路瓶頸 | network bottlenecks |
| 為帳單計算的IP計量 | IP accounting for billing purposes |
| 增加網路性能 | increase network performance  |
| 減少網路負載 | reduce the network load |
| 限制路由更新傳送 | restrict the delivery of routing updates |
| 阻擋影片流量的網路存取控制清單 | ACLs that block video traffic |
| 對HR網路範圍存取可以被限制在有授權的用戶。 | access to the Human Resources network can be restricted to authorized users |
| 網路存取控制清單掃描主機允許、拒絕對網路服務的存取 | ACLs screen hosts to permit or deny access to network services |
| 這個ACL能力類似在演唱會、運動會擁有VIP通行證 | This ACL capability is similar to having a VIP pass at a concert or sporting event. |
| 更精細的控制 | finer control |
| 符合允許或拒決規則 | meet the permit or deny criteria  |
| 使用編號或名稱分辨ACL和命令列表 | use either a number or a name to identify the ACL and its list of statements |
| 根據需要 | as (is) required |
