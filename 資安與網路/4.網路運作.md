...
2. 傳輸層協定
    - TCP (Transmission Control Protocol)
        ...
        - 三向交握(3-way handshake process)：host A驗證host B。全雙工協定(full-duplex protocol)
            - 視窗大小(Window Size)是傳送端不需等待ACK可傳送封包的大小，接收端在三向交握時通知發送端的，過程若有調整稱為 sliding window。
                - TCP 的 Congestion Avoidance (壅塞避免) 機制：提前降低傳輸速度，以避免封包大量遺失，屬於「傳輸層」的機制。cf. 與 CSMA/CA、CD 討論 OSI 層級「資料連結層」不同。
...
2. DNS (Domain Name System)
    ...
    - 屬於階層化管理，DNS domain hierarchy
        ...
        - 子網域 subdomains
    - DNS Server快取功能 (Caching)
        - 減少對上層 DNS 或權威伺服器的請求，降低網路負擔，減少查詢延遲。
