...
- TCP/IP Protocol Suite
    ...
    - Application Layer
        ...
        - **DHCPv4 (Dynamic Host Configuration Protocol for IPv4) / DHCPv6 (Dynamic Host Configuration Protocol for IPv6)**：自動分配 IPv4/IPv6 位址
    ...
    - Internet Layer
        ...
        - **ICMPv4 (Internet Control Message Protocol for IPv4)**：提供錯誤回報與診斷
        - **ICMPv6 (Internet Control Message Protocol for IPv6)**：IPv6 的控制訊息協定
        - **ICMPv6 ND (Neighbor Discovery)**：位址解析與重複位址檢測

    - Routing Protocols
        - **OSPF (Open Shortest Path First)**：開放標準的鏈路狀態路由協定
        - **EIGRP (Enhanced Interior Gateway Routing Protocol)**：Cisco 專有的路由協定
        - **BGP (Border Gateway Protocol)**：ISP 之間使用的外部路由協定

    - Network Access Layer
        - **ARP (Address Resolution Protocol)**：將 IPv4 位址對應到 MAC 位址
        - **Ethernet**：有線區域網路標準
