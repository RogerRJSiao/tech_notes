...
- 檔案系統
    ...
    - 查檔案權限(file permissions)用 `ls -l` 指令。
        | 第一欄第1碼 | 第一欄第2~10碼 | 第二欄 | 第三四欄 | 第五欄 | 第六七八欄 | 第八欄 |
        | -- | -- | -- | -- | -- | -- | -- |
        | -是file，d是dir | read/write/execute for **user, group, others** | 2=hard link, symbolic link | 檔案擁有的user+group | bytes | 最後修改時刻 | 檔名 |
        - 建立強連結(hard link)檔案，用 `ln space.txt space.hard.txt` 指令。原檔會隨新檔增減內容，但不會因新檔rm移除而消失。
            - 特點：1.只能在同一個磁區(Partition)中建立，2.也只能連結檔案，不能連結目錄。3.所有強連結都指向同一個inode，在檔案系統中採不同檔名建立，沒有原始、副本之分。
        - 建立軟連結(symbolic/soft link, symlinks)檔案，用 `ln -s test.txt mytest.txt` 指令。類似MS-Windows下的「建立捷徑」的功能，主要用於簡化路徑。直接連上原檔路徑，故原檔被刪除時，新檔也會讀取錯誤。
            - 優點：1.用於跨檔案系統(=磁區)的連結。2.目錄間的連結。3.儲存路徑資訊，可查來源位址。
...
- Windows架構和作業
    ...
    - Windows設定與執行
        ...
        - 檢查及設定網路
            ...
            - `nslookup cisco.com` 連線DNS server查詢並解析DNS位址、紀錄。
